(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const v of s.addedNodes)v.tagName==="LINK"&&v.rel==="modulepreload"&&o(v)}).observe(document,{childList:!0,subtree:!0});function i(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(n){if(n.ep)return;n.ep=!0;const s=i(n);fetch(n.href,s)}})();const L=[{href:"#uvod",label:"Úvod"},{href:"#sluzby",label:"Služby"},{href:"#realizace",label:"Realizace"},{href:"#omne",label:"Kdo k vám přijede"},{href:"#kontakt",label:"Kontakt"}],I="+420 725 414 555",S="tel:+420725414555",O="420725414555";let u=!1;function E(){const e=document.querySelector(".nav-links");e&&(u=!u,e.classList.toggle("mobile-open",u))}function P(){return`
    <header class="navbar">
  ...
  `}function H(){const e=document.getElementById("gallery-tabs"),t=document.getElementById("gallery-content"),i=document.getElementById("lightbox"),o=document.getElementById("lightbox-img"),n=document.getElementById("lightbox-close"),s=document.getElementById("lightbox-prev"),v=document.getElementById("lightbox-next");if(!e||!t)return;function j(){const a=`<button class="gallery-tab ${k==="all"?"active":""}" data-category="all">Vše (Průřez)</button>`,l=Object.entries(h).map(([r,p])=>`
      ...
      `,t.querySelectorAll(".gallery-card").forEach(r=>{r.addEventListener("click",()=>{f(parseInt(r.dataset.index,10))})})}}function f(a){console.log("Opening lightbox index:",a),!(a<0||a>=c.length)&&(d=a,m(),i.classList.add("visible"),document.body.style.overflow="hidden")}function b(){i.classList.remove("visible"),document.body.style.overflow=""}function m(){const a=c[d];a&&(o.src=a.src,o.alt=a.alt||"")}function z(){d=(d+1)%c.length,m()}function x(){d=(d-1+c.length)%c.length,m()}e.addEventListener("click",a=>{a.target.classList.contains("gallery-tab")&&y(a.target.dataset.category)}),i&&(n.addEventListener("click",b),v.addEventListener("click",a=>{a.stopPropagation(),z()}),s.addEventListener("click",a=>{a.stopPropagation(),x()}),i.addEventListener("click",a=>{(a.target===i||a.target.classList.contains("lightbox-content"))&&b()}),document.addEventListener("keydown",a=>{i.classList.contains("visible")&&(a.key==="Escape"&&b(),a.key==="ArrowRight"&&z(),a.key==="ArrowLeft"&&x())})),y("all")}function M(){return`
  ...
  `}function G(){return localStorage.getItem("cookie-consent")?"":`
  ...
  `}function Y(){const e=document.getElementById("cookie-banner"),t=document.getElementById("cookie-accept"),i=document.getElementById("cookie-reject"),o=document.getElementById("cookie-settings");if(!e)return;const n=s=>{localStorage.setItem("cookie-consent",s),e.classList.add("cookie-hidden"),setTimeout(()=>e.remove(),400),s==="all"&&window.dispatchEvent(new CustomEvent("cookie-consent-all"))};t==null||t.addEventListener("click",()=>n("all")),i==null||i.addEventListener("click",()=>n("essential")),o==null||o.addEventListener("click",()=>{alert("Zde by byla podrobná nastavení cookies. Pro tento web používáme pouze technické a analytické cookies.")})}console.log("Main.js loaded");function F(){return`
    ${P()}
    ${T()}
    ${C()}
    ${K()}
    ...
    ${q()}
    ${G()}
  `}function U(){$(),A(),H(),Z(),Y();const e={root:null,threshold:.15,rootMargin:"0px"},t=new IntersectionObserver((i,o)=>{i.forEach(n=>{n.isIntersecting&&(n.target.classList.add("visible"),o.unobserve(n.target))})},e);document.querySelectorAll("section:not(#uvod)").forEach(i=>{i.classList.add("reveal"),t.observe(i)})}function J(){try{const e=document.getElementById("app");e&&(e.innerHTML=F(),U())}catch(e){console.error("CRITICAL ERROR:",e),document.body.innerHTML=`<div style="color: red; padding: 20px; text-align: center;">
